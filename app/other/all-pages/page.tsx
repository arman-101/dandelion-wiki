...
export default function AllPages() {
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-8">
            <h1 className="text-4xl font-bold text-center mb-12 text-gray-900 dark:text-white">Complete Site Index</h1>
                    ...
                    <div key={type}>
                        ...
                        <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-3">
                            {groupedPages[type].map(page => (
                                <li key={page.path}>
                                    <Link href={page.path} className="text-teal-600 dark:text-teal-400 hover:underline hover:text-[--color-accent-pink)] transition-colors">
                                        {page.title}
                                    </Link>
                                </li>
                            ))}
