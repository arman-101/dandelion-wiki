...
export default function InfoBox({ title, data, className = "" }: InfoBoxProps) {
        ...
        if (Array.isArray(value)) {
            return value.map((item, index) => (
                <Fragment key={`${item.text}-${index}`}>
                    {item.link && item.link !== "" ? (
                        <Link href={item.link} className="text-link dark:text-link-dark hover:underline">
                            {item.text}
                        </Link>
                    ) : (
                        <span>{item.text}</span>
            ...
            return value.link && value.link !== "" ? (
                <Link href={value.link} className="text-link dark:text-link-dark hover:underline">
                    {value.text}
                </Link>
            ) : (
    ...
    return (
        <div className={`bg-bg-card dark:bg-bg-card p-4 rounded-lg shadow-md border border-border-secondary dark:border-border-primary ${className}`}>
            <h3 className="font-bold text-lg mb-3 text-link dark:text-link-dark border-b border-gray-300 dark:border-border-secondary pb-2">
                {title}
            </h3>
            <dl className="space-y-2 text-sm text-text-secondary dark:text-text-secondary">
