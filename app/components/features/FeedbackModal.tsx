...
export default function FeedbackModal({ isOpen, onClose }: FeedbackModalProps) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
            <div className="bg-bg-card dark:bg-bg-card rounded-lg shadow-2xl p-6 w-full max-w-md relative">
                <button onClick={onClose} className="absolute top-3 right-3 text-text-light hover:text-text-muted dark:hover:text-gray-200 transition-colors">
                    <XIcon />
                </button>
                <h2 className="text-2xl font-bold mb-4 text-text-primary dark:text-text-primary">Give Feedback</h2>
                ...
                {status ? (
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="mb-4">
                            <label htmlFor="email" className="block text-sm font-medium text-text-secondary dark:text-text-secondary mb-1">Your Email</label>
                            <input 
                                type="email" 
                                id="email" 
                                value={email} 
                        ...
                        <div className="mb-6">
                            <label htmlFor="message" className="block text-sm font-medium text-text-secondary dark:text-text-secondary mb-1">Details</label>
                            <textarea 
                                id="message" 
                                rows={4} 
                                value={message} 
                        ...
                        <div className="flex justify-end gap-4">
                            ...
                            <button type="submit" className="px-4 py-2 bg-primary text-white rounded-md hover:bg-primary-dark transition-colors">Submit</button>
                        </div>
                    </form>
                )}
            </div>
